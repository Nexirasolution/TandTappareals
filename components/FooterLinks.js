...
function AccordionSection({ id, title, openSection, onToggle, children }) {
  ...
  return (
    <div className="border-b sm:border-0" style={{ borderColor: LINE }}>
      ...
      >
        <span className="font-medium text-[11px] tracking-wide uppercase" style={{ color: BLACK_SOFT }}>
          {title}
        </span>
        <ChevronDown
          size={14}
          className="sm:hidden transition-transform duration-200"
          style={{ color: BLACK_SOFT, transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}
        />
      </button>
      ...
      <div className="sm:col-span-2">
        <AccordionSection id="shop" title="Shop" openSection={openSection} onToggle={toggle}>
          <ul className="space-y-2.5">
            {categories.map((cat) => (
              <li key={cat.slug}>
                <Link href={`/category/${cat.slug}`} className="text-[13px] transition-colors" style={{ color: BLACK }}>
      ...
      <div className="sm:col-span-3">
        <AccordionSection id="quicklinks" title="Quick Links" openSection={openSection} onToggle={toggle}>
          <ul className="space-y-2.5">
            {quickLinks.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-[13px] transition-colors" style={{ color: BLACK }}>
        ...
        <AccordionSection id="connect" title="Connect" openSection={openSection} onToggle={toggle}>
          <div className="flex gap-2 mb-5">
            <a href={`https://wa.me/${whatsapp}`}
              ...
              style={{ background: 'transparent', border: `1px solid ${LINE}`, color: BLACK }}
            >
