...
const STATUS_STYLES = {
  placed:    { color: INK_SOFT,    background: '#F0F0F0' },
  confirmed: { color: RED,         background: RED_LIGHT },
  packed:    { color: RED,         background: RED_LIGHT },
  shipped:   { color: DEEP_RED,    background: RED_WASH },
  delivered: { color: ACCENT_DARK, background: '#F0F0F0' },
  cancelled: { color: DEEP_RED,    background: '#FBE0E3' },
  returned:  { color: INK_SOFT,    background: '#F0F0F0' },
};
...
function ReviewForm({ order, item, phone, onDone }) {
  ...
  return (
    <div className="p-3.5 mt-2" style={{ background: RED_LIGHT, borderRadius: '4px' }}>
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <button key={i} type="button" onClick={() => setRating(i + 1)}>
            <Star
              size={18}
              strokeWidth={1.5}
              style={{ color: RED, fill: i < rating ? RED : 'transparent' }}
            />
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        ...
        <label
          ...
          style={{ border: `1px dashed ${RED}`, borderRadius: '3px', color: INK_SOFT }}
        >
      ...
      <button
        ...
        style={{ background: RED, color: PAPER, borderRadius: '3px' }}
      >
...
export default function OrdersPage() {
  ...
  return (
    <div className="max-w-xl mx-auto px-5 py-10 sm:py-14 min-h-[60vh]" style={{ background: PAPER }}>
      ...
      {error && <p className="text-sm mb-4" style={{ color: DEEP_RED }}>{error}</p>}

      {orders !== null && orders.length === 0 && !error && (
        <div className="text-center py-16">
          <Package size={26} strokeWidth={1.5} className="mx-auto mb-3" style={{ color: RED, opacity: 0.5 }} />
      ...
      <div className="space-y-4 mt-6">
        {orders?.map((o) => {
          const statusStyle = STATUS_STYLES[o.status] || { color: INK_SOFT, background: '#F0F0F0' };
          return (
                  ...
                  return (
                    <div key={i}>
                      <div className="flex items-center gap-2.5">
                        <div className="w-11 h-11 overflow-hidden shrink-0 relative" style={{ background: RED_WASH, borderRadius: '3px' }}>
                        ...
                        {canReview && (
                          alreadyReviewed ? (
                            <span className="flex items-center gap-1 text-[11px] shrink-0" style={{ color: ACCENT_DARK }}>
