    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {category?.parent && (
        <Link
          ...
          className="text-sm text-neutral-500 hover:text-red-600 inline-flex items-center gap-1 mb-3"
        >
      ...
      {subcategories.length > 0 && (
        <div className="flex gap-3 overflow-x-auto pb-2 mb-6 -mx-1 px-1">
          {subcategories.map((sub) => (
            ...
            >
              <div className="w-24 h-24 rounded-full overflow-hidden bg-red-50 border border-neutral-200">
                {sub.image && (
              ...
              <span className="text-xs font-medium text-neutral-800 text-center leading-tight">
                {sub.name}
              </span>
            </Link>
          ))}
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="aspect-[3/4] rounded-xl bg-red-50 border border-neutral-200 animate-pulse" />
          ))}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center py-20">
          <span className="text-4xl">🛍️</span>
          <p className="mt-3 text-base font-medium text-black">No products found in this category yet.</p>
