    ...
    <div className="max-w-6xl mx-auto px-5 sm:px-8 py-10 sm:py-16 min-h-[60vh]" style={{ background: PAPER }}>
      ...
      {loading && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="aspect-[3/4]" style={{ background: RED_WASH, borderRadius: '4px' }} />
          ))}
        </div>
      )}
      ...
      {!loading && products.length === 0 && (
        <div className="text-center py-20">
          <Heart size={28} strokeWidth={1.5} className="mx-auto mb-3" style={{ color: RED, opacity: 0.5 }} />
